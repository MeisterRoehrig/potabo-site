...
import { getLucideIcon } from '@/components/Icon/LucideIcon'
    ...
    <section className="py-32">
      <div className=" border-y">
        <div className="container flex flex-col gap-6 border-x py-4 max-lg:border-x lg:py-8">
          <h2 className="text-3xl leading-tight tracking-tight md:text-4xl lg:text-6xl">{title}</h2>
          <p className="max-w-[600px] tracking-[-0.32px] text-muted-foreground">{description}</p>
        </div>
      </div>
      ...
      <div className="container overflow-hidden border-x pb-40 lg:pt-20 [&>*:last-child]:pb-20 [&>div>div:first-child]:pt-20!">
        {items &&
          ...
          items.map((item, index) => {
            ...
            const iconConfig = icon as unknown as LucideIconPickerType
            const IconComponent = getLucideIcon(iconConfig)

            return (
              <div key={index} className="relative flex">
                <div
                  className={`flex w-full justify-center px-1 py-10 text-end md:gap-6 lg:gap-10 ${index % 2 ? 'lg:flex-row-reverse lg:text-start' : ''} `}
                >
                  <div className="flex-1 max-lg:hidden">
                    <h3 className="text-2xl tracking-[-0.96px]">{heading}</h3>
                    <p
                      className={`mt-2.5 max-w-[300px] tracking-[-0.32px] text-balance text-muted-foreground ${index % 2 ? '' : 'ml-auto'}`}
                    >
                      {description}
                    </p>
                  </div>
                  ...
                  <div className="flex-1 max-lg:-translate-x-4">
                    ...
                    <div className="flex items-start justify-start">
                      <div className={` ${index % 2 ? 'lg:ml-auto' : ''}`}>
                ...
                <div
                  className={`absolute z-[-2] h-full w-[3px] translate-x-5 rounded-full lg:left-1/2 lg:-translate-x-1/2 ${index === items.length - 1 ? 'bg-linear-to-b from-foreground/10 via-foreground/10 to-transparent' : 'bg-foreground/10'}`}
                >
                  {index == 0 && (
